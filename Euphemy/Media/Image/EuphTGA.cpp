...
public:
	...
	struct ExtensionInformation {
		...
		void load(Elv::Io::DataStream<Elv::Util::Endian::Little>& input);
	};
	...
	// Commands
	void load(Elv::Io::Device& input);
	Header(std::pmr::memory_resource* memRes) : colorMap(memRes), imageData(memRes) {

	}
private:
	void decodeImage(size_t imageSize, Elv::Io::Device& input);
	void decodeCompressedImage(size_t imageSize, Elv::Io::Device& input);
	void flipVert();
	void flipHoriz();

};

void Header::load(Elv::Io::Device &input)
{
	Elv::Io::DataStream<Elv::Util::Endian::Little> tgaInput(input); // TGA is little-endian.
	...
	input.seek(idLen,Elv::Io::SeekOrigin::CUR); // Skip ID.
	if(colmapType) {
		...
		input.read(colorMap.data(),sizeof(std::byte),colMapSize);
	}
	size_t imageSize = imageSpecification.width * imageSpecification.height * (imageSpecification.pixelDepth / 8);
	if(isCompressed) decodeCompressedImage(imageSize,input);
	else decodeImage(imageSize, input);
...
void Header::decodeImage(size_t imageSize, Elv::Io::Device& input)
{
	imageData.resize(imageSize);
	input.read(imageData.data(),sizeof(std::byte),imageSize); // Will read past the end of file in case of RLE.
}

void Header::decodeCompressedImage(size_t imageSize, Elv::Io::Device &input)
{
	imageData.resize(imageSize);
	input.read(imageData.data(),sizeof(std::byte),imageSize); // Will read past the end of file in case of RLE.
...
void Header::ExtensionInformation::load(Elv::Io::DataStream<Elv::Util::Endian::Little> &input)
{
	input >> extensionSize;
	input >> authorName;
	input >> authorComment;
...
void decode(Elv::Io::Device& iodev, DecodeTarget& destination) {
	Header head(destination.getMemResource());
	head.load(iodev);
	switch (head.format) {
