...
void Font::insertCharacter(char32_t c)
{
	...
	// Now we do something with all these stuff
	Character character = {
		.valid = true,
		.offset = glyphOffset,
		.size = glyphSize,
	...
	// This is also intended to increment the texture offset... probably.
	insertCharacterIntoBackend(character,bytes,glyphSize,glyphBearing,glyphAdvance,glyphOffset,intendedCorner); // might modify it, due to padding
	// Now we insert
...
unsigned long IoFromMh33( FT_Stream       stream,
						  ...
						  unsigned long   count ) {
	MH33::Io::Device* iodev = static_cast<MH33::Io::Device*>(stream->descriptor.pointer);
	iodev->seek(MH33::Io::SeekOrigin::SET,offset);
	return iodev->read(buffer,count);
}
void CloseIoFromMh33( FT_Stream  stream ) {
	(void)stream;
}

Font::Font(MH33::Io::uDevice&& iodev, const sFreeTypeSystem& system, unsigned fontSize, bool bold, bool isSdf)
	  ...
	  isBold(bold), isSdf(isSdf), textureOffset(0,1), maxCharSizeSoFar(0,0)
{
	...
	stream.size = this->iodev->size();
	stream.descriptor = { .pointer = this->iodev.get() };
	memset(&openArgs,0,sizeof(FT_Open_Args_));
...
void Font::renderTextBlocks(const std::span<const TextBlockUtf32> textBlocks, const glm::fvec2& offset, const glm::fvec2& reciprocalSize, float scale, int spacing)
{
	TextRenderState state;
	memset(&state,0,sizeof(TextRenderState));
	state.originalOffset = offset;
	...
	for(const auto& block : textBlocks) {
		...
		if(block.font) block.font->renderText(block.text,state);
	}
}

const Font::CharacterMap& Font::getCharacterMap() const
{
	return characters;
}
